...
export abstract class Transformer {
    ...
    public abstract reverse(
        ...
        continuationTransformer: Transformer | undefined,
    ): Transformer;

    public abstract reconstitute<TBuilder extends BaseGraphRewriteBuilder>(
        builder: TBuilder,
    ): Transformer;
...
export class DecodingTransformer extends ProducerTransformer {
    ...
    public reverse(
        ...
        if (continuationTransformer !== undefined) {
            return panic(
                "Reversing a decoding transformer cannot have a continuation",
            );
        }
...
export class EncodingTransformer extends Transformer {
    ...
    public reverse(
        _targetTypeRef: TypeRef,
        _continuationTransformer: Transformer | undefined,
    ): Transformer {
    ...
    public reconstitute<TBuilder extends BaseGraphRewriteBuilder>(
        ...
        return new EncodingTransformer(
            builder.typeGraph,
            builder.reconstituteTypeRef(this.sourceTypeRef),
        );
    }
...
export class ArrayDecodingTransformer extends ProducerTransformer {
    ...
    public getNumberOfNodes(): number {
        return (
            super.getNumberOfNodes() + this.itemTransformer.getNumberOfNodes()
        );
    }
    ...
    public reverse(
        ...
        const itemTransformer = this.itemTransformer.reverse(
...
export class ChoiceTransformer extends Transformer {
    public constructor(
        ...
        assert(
            ...
            "Choice must have at least one transformer",
        );
    }
    ...
    public reverse(
        ...
        const transformers = this.transformers.map((xfer) =>
            xfer.reverse(targetTypeRef, continuationTransformer),
        );
        if (
            transformers.every(
                (xfer) => xfer instanceof UnionMemberMatchTransformer,
            )
        ) {
            const memberMatchers =
                transformers as UnionMemberMatchTransformer[];
            const first = memberMatchers[0];
            if (
                memberMatchers.every((xfer) =>
                    first.memberType.equals(xfer.memberType),
                )
            ) {
                const subTransformers = memberMatchers.map(
                    (xfer) => xfer.transformer,
                );
                return new UnionMemberMatchTransformer(
                    ...
                    new ChoiceTransformer(
                        this.graph,
                        subTransformers[0].sourceTypeRef,
                        subTransformers,
                    ),
                    first.memberTypeRef,
                );
    ...
    public reverse(
        ...
        function filter(xfers: Transformer[]): Transformer[] {
            ...
            const smallest = arraySortByInto(
                nonfailing.map(
                    (x) => [x.getNumberOfNodes(), x] as [number, Transformer],
                ),
                ([c, _]) => c,
            )[0][1];
            return [smallest];
        ...
        const allTransformers = Array.from(transformers.values()).concat(
            Array.from(memberMatchTransformers.values()),
        );
        const resultingTransformers = ([] as Transformer[]).concat(
            ...allTransformers.map(filter),
        );
...
export class UnionMemberMatchTransformer extends MatchTransformer {
    ...
    public get sourceType(): UnionType {
        ...
        if (!(t instanceof UnionType)) {
            return panic(
                "The source of a union member match transformer must be a union type",
            );
        }
...
export class StringMatchTransformer extends MatchTransformer {
    ...
    public get sourceType(): EnumType | PrimitiveType {
        ...
        if (
            ...
            !(t instanceof PrimitiveType && t.kind === "string")
        ) {
            return panic(
                "The source of a string match transformer must be an enum or string type",
            );
        }
...
export class UnionInstantiationTransformer extends Transformer {
    ...
    public reverse(
        ...
        if (continuationTransformer === undefined) {
            return panic(
                "Union instantiation transformer reverse must have a continuation",
            );
        }
...
export class StringProducerTransformer extends ProducerTransformer {
    ...
    public reverse(
        ...
        if (continuationTransformer === undefined) {
            return panic(
                "Reversing a string producer transformer must have a continuation",
            );
        }
...
export class Transformation {
    ...
    public equals<T extends Transformer>(other: T): boolean {
        ...
        return (
            this._targetTypeRef === other._targetTypeRef &&
...
export const transformationTypeAttributeKind: TypeAttributeKind<Transformation> =
    new TransformationTypeAttributeKind();

export function transformationForType(t: Type): Transformation | undefined {
    return transformationTypeAttributeKind.tryGetInAttributes(
        t.getAttributes(),
    );
}
