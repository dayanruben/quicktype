...
function shouldBeMap(
    ...
    if (
        ...
        iterableEvery(
            properties.values(),
            (cp) =>
                isPrimitiveStringTypeKind(cp.type.kind) ||
                cp.type.kind === "null",
        )
    ) {
        return undefined;
            ...
            if (firstNonNullCases !== undefined) {
                ...
                if (
                    !setOperationCasesEqual(
                        ...
                        firstNonNullCases,
                        true,
                        (a, b) => a.structurallyCompatible(b, true),
                    )
                ) {
                    canBeMap = false;
                    break;
...
export function inferMaps(
    ...
    function replaceClass(
        ...
        if (shouldBe === undefined) {
            return panic(
                `We shouldn't be replacing class ${c.getCombinedName()} with a map`,
            );
        }
