...
export class ElixirRenderer extends ConvenienceRenderer {
    ...
    protected forbiddenNamesForGlobalNamespace(): string[] {
        return [
            ...
            ...reservedWords.map((word) => capitalizeFirstLetter(word)),
        ];
    }
    ...
    private elixirType(t: Type, isOptional = false): Sourcelike {
        ...
        return matchType<Sourcelike>(
            ...
            (unionType) => {
                const children = [...unionType.getChildren()].map((ut) =>
                    this.elixirType(ut),
                );
                return [
    ...
    protected emitPatternMatches(
        ...
        if (typesToMatch.find((type) => type.kind === "double")) {
            typesToMatch = typesToMatch.filter(
    ...
    protected emitModule(c: ClassType, moduleName: Name): void {
        this.emitBlock(
            ["defmodule ", this.nameWithNamespace(moduleName), " do"],
            () => {
                ...
                this.forEachClassProperty(c, "none", (name, _jsonName, p) => {
                    if (p.type.kind === "union") {
                        ...
                        const unionTypesNonNull = unionTypes.filter(
                            (type) => type.kind !== "null",
                        );
                        if (unionTypesNonNull.length === 1) {
                            ...
                            if (unionTypesNonNull[0] instanceof ArrayType) {
                                ...
                                unionTypesNonNull[0] instanceof MapType
                            ) {
                                suffix = "_value";
                                itemTypes = [
                                    ...unionTypesNonNull[0].getChildren(),
                            ...
                            if (
                                itemTypes.length === 1 &&
                                itemTypes[0] instanceof UnionType
                            ) {
